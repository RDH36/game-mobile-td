...
public class EnemySpawner : MonoBehaviour
{
    ...
    public void SpawnWave(WaveEntry[] entries)
    {
        ...
        foreach (WaveEntry entry in entries)
        {
            ...
            {
                Vector2 pos = FindValidPosition(arenaMinX, arenaMaxX, arenaMinY, arenaMaxY, usedPositions);
    ...
    Vector2 FindValidPosition(float minX, float maxX, float minY, float maxY, List<Vector2> used)
    {
        for (int attempt = 0; attempt < 50; attempt++)
        {
            ...
            {
                if (Vector2.Distance(candidate, p) < minSpacingBetweenEnemies)
                {
                    valid = false;
                    break;
                }
